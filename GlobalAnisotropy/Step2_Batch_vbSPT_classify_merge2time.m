...
%   Person F,Lind�n M, Unoson C, Elf J, Extracting intracellular reaction rates from single molecule tracking data, Nature Methods 10, 265?269 (2013). doi:10.1038/nmeth.2367
...
MinTrajLength = 2;
input_path = [resultDir, filesep, 'QC_data', filesep];
path_reformatted = [resultDir, filesep, 'QC_data_reformatted', filesep];
path_classified = [resultDir, filesep, 'HMM_first_QC_data', filesep];
maxHidden = 2;
...
for FileIter = 1:length(Filenames)
    ...
    for i = 1:length(trackedPar_QC)
        if size(trackedPar_QC(i).xy,1) >= MinTrajLength
            ...
            %ZW comment: the maximum gap allowed when do LocAndTrack is 1,
            %so if there is any gaps, diff must be 2.
...
for FileIter = 1:length(Filenames)
    ...
    if exist([resultDir, '/FolderWithRunInputFiles/vbSPT_RunInputFileBatch', num2str(FileIter), '.m'])
        delete([resultDir, '/FolderWithRunInputFiles/vbSPT_RunInputFileBatch', num2str(FileIter), '.m']);
    end
    ...
    copyfile('vbSPT_RunInputFileBatch.m', [resultDir, '/FolderWithRunInputFiles/vbSPT_RunInputFileBatch', num2str(FileIter), '.m']);
    
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%%%%%%%%%%%%%%%%% Run vbSPT on the data %%%%%%%%%%%%%%%%%%
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    R=VB3_HMManalysis([resultDir, '/FolderWithRunInputFiles/vbSPT_RunInputFileBatch', num2str(FileIter), '.m']);
